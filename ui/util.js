class Util {
	...
	static create_element(opt, parent) {
		if(opt.condition === false) {
			return;
		}
		let el = opt.type ? document.createElement(opt.type) : document.createDocumentFragment(),
			optToProp = {
			'src' 			: 'src',
			'text'			: 'textContent',
			'val'			: 'value',
		...
		if(opt.children && opt.children instanceof Array) {
			opt.children.forEach(function(c) {
				Util.create_element(typeof c === 'string' ? {
					type: 'span',
					text: c
				} : c, el);
			});
	...
	static color_hash(str, hue, sat, val) {
	...
	static load_modules(modules, directory, obj) {
		modules.forEach(function(el) {
			obj[el] = require(`${directory}/${el}.js`);
		});
	}
}
